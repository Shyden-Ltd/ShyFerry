...
hits = [(i + 1, line.strip()) for i, line in enumerate(lines) if PLACEHOLDER.search(line)]
for ln, line in hits:
    problems.append(f"placeholder at line {ln}: {line[:70]}")
...
stories = sorted(int(m.group(2)) for m in (STORY.match(line) for line in lines) if m)
...
for line in lines:
    for m in DECISION.finditer(line):
        (d_defined if line.lstrip().startswith(f"| D{m.group(1)} |") else d_cited).add(
            int(m.group(1))
        )
...
control(
    "inv mutation, negative",
    inv_row_lacks_mutation,
    "| INV-9 | claim | how | remove the control |",
    expect=False,
)
for i, line in enumerate(lines):
    if line.lstrip().startswith("| INV-") and inv_row_lacks_mutation(line):
        problems.append(f"INV row at line {i + 1} has no mutation: {line.strip()[:48]}")
...
for i, line in enumerate(lines):
    if line.startswith("shyferry "):  # the fenced CLI surface block
        surface.add(line.split()[1])
        in_block = True
        continue
...
for cmd, where in sorted(mentioned.items()):
    if cmd not in surface:
        problems.append(
            f"command 'shyferry {cmd}' used at line(s) {where} is absent from the CLI surface"
        )
...
for i, line in enumerate(lines):
    if STORY.match(line):
        continue
    for m in STORY_REF.finditer(line):
        if int(m.group(1)) not in stories:
            problems.append(
                f"S-{m.group(1)} cited at line {i + 1} but absent from the story breakdown"
            )
...
for line in lines:
    ...
    if not m:
        continue
    sid, ranges = m.group(1), list(re.finditer(r"INV-(\d+) to INV-(\d+)", line))
    for rng in ranges:
        for n in range(int(rng.group(1)), int(rng.group(2)) + 1):
            owners.setdefault(n, []).append(sid)
    for single in re.finditer(r"INV-(\d+)", re.sub(r"INV-\d+ to INV-\d+", "", line)):
...
if fences_unbalanced("\n" + text):
    problems.append(
        "code fences do not balance; every renderer will swallow the rest of the document"
    )
...
control(
    "orphan identifier, negative",
    lambda s: cited_once_only(s, "R-99"),
    "R-99 defined here and R-99 cited there",
    expect=False,
)
